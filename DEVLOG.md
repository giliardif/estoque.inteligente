...
## Etapa 37 — Tela de Configurações (backend): Empresa, Minha Conta, e correção de RLS em `tenants`

**Contexto:** pedido do usuário por uma tela de Configurações unificada, com abas. Mapeado com preview HTML interativo antes de qualquer código (múltiplas rodadas de ajuste em conversa — escopo final: Minha Conta, Empresa, Usuários, Estações de Impressão). Auditoria do código real revelou que 2 das 4 abas (Minha Conta, Empresa) não tinham backend nenhum: `tenants` só tinha `nome`/`segmento_slug` (sem CNPJ, sem endpoint GET/PATCH), e não existia rota de usuário editar o próprio nome ou enviar foto — só `PATCH /usuarios/{id}` (admin gerenciando outro usuário). Usuários e Estações de Impressão migram para dentro de Configurações e saem do menu principal; Etiquetas foi deliberadamente excluída do escopo (é ferramenta operacional pesada — busca de produto, scanner, preview, impressão — não um formulário de configuração).

Esta entrada cobre só o **backend** das 3 pendências identificadas (Empresa, nome próprio, foto de perfil) e uma **correção de segurança** encontrada no processo. O frontend fica para a próxima entrada desta mesma etapa.

**Backend — 3 pendências:**
- Migration `014_configuracoes.sql`: `tenants.cnpj` (nullable, texto — validação de dígito verificador na aplicação, não no schema) e `users.avatar_url` (nullable, segue o mesmo padrão de `produtos.imagem_url`).
- Módulo `tenant/` novo: `GET /api/v1/tenant` (qualquer perfil lê — a tela de Configurações é comum a todos) e `PATCH /api/v1/tenant` (só admin edita). `segmento_slug` não é editável por aqui de propósito — é fixado no onboarding e usado por outras partes do sistema (categorias padrão do segmento); mudar isso é decisão de produto maior, fora de escopo. CNPJ validado por algoritmo real de dígito verificador (`validar_cnpj`), não só contagem de 14 caracteres — um CNPJ com formato certo mas DV errado é rejeitado com 422.
- `GET/PATCH /api/v1/usuarios/me` (editar o próprio nome — qualquer perfil). Schema `UsuarioMeUpdate` separado de `UsuarioUpdate` de propósito: são superfícies de permissão diferentes (autoatendimento vs. admin gerenciando outro usuário), e reaproveitar o mesmo schema abriria a porta pra um dia alguém adicionar `perfil`/`ativo` nele por engano. Rotas `/me` e `/me/foto` registradas ANTES de `/{usuario_id}` no router — necessário porque o FastAPI casa rotas pela estrutura do path antes de validar tipo, então `/me` bateria no parâmetro `usuario_id: UUID` e falharia com 422 se viesse depois.
- `POST /api/v1/usuarios/me/foto`: `app/core/storage.py` foi generalizado (extraído `_enviar_imagem` como núcleo comum parametrizado por bucket/caminho/limite; `enviar_imagem_produto` continua com a mesma assinatura e comportamento de antes) e ganhou `enviar_imagem_usuario`. Bucket novo (`usuarios-avatares`, setting `SUPABASE_STORAGE_BUCKET_AVATARES`) — **precisa ser criado manualmente no Supabase Storage antes de funcionar em staging/produção**, mesma exigência que já valia pro bucket de produtos. Limite de 2MB (`MAX_IMAGEM_AVATAR_SIZE_MB`), abaixo do de produto (3MB) — avatar não precisa da mesma resolução.
- `nome` no JWT da sessão atual só atualiza no próximo login/refresh — mesma limitação já documentada pra outros campos do token (`deve_trocar_senha`). `avatar_url` não entra no JWT (ficaria bloated e exigiria reemitir token a cada troca de foto); frontend busca via `GET /usuarios/me`.

**Correção de segurança encontrada durante a etapa: RLS habilitada em `tenants`.**
`tenants` ficou de fora da RLS desde a migration 001 por decisão consciente — nenhum código de aplicação fazia consulta por-tenant nela no dia a dia (só o cadastro inicial, via `auth_service`, que já tem bypass escopado a essa tabela). Risco era avaliado como zero. Isso mudou nesta etapa: agora existe `GET/PATCH /tenant`, endpoint real rodando pela conexão normal da aplicação (sem bypass) toda vez que a tela de Configurações abre. A partir daqui, a segurança desse endpoint passaria a depender inteiramente do filtro `WHERE id = tenant_id` em `tenant/service.py` nunca ser esquecido por um dev futuro — diferente das outras 19 tabelas do sistema, onde o Postgres pega esse tipo de bug sozinho via `FORCE ROW LEVEL SECURITY`.

- Migration `015_rls_tenants.sql`: `ENABLE`/`FORCE ROW LEVEL SECURITY` em `tenants` + policy `tenant_isolation` usando `id` (não `tenant_id` — não existe essa coluna aqui, o `id` da própria linha É o tenant_id usado no resto do sistema). `auth_service` não foi afetado — seu bypass já era escopado para incluir `tenants`.
- **Prova adversarial real** (não só leitura de policy): com o role de aplicação (`estoque_app_test` local / `app_estoque` em staging, sem `BYPASSRLS`), setando `app.tenant_id` para um UUID aleatório — simulando um filtro esquecido no código — a query direta em `tenants` retornou **0 linhas**, mesmo com 287 tenants reais na tabela. Com o `tenant_id` correto, retornou exatamente 1. Sem a correção, esse mesmo cenário teria vazado as 287 linhas.
- Aplicada em produção... digo, em staging via Supabase MCP, com a verificação de 3 passos de sempre: `pg_class` (`relrowsecurity`/`relforcerowsecurity` = true), `pg_policies` (policy `tenant_isolation` presente, `USING`/`WITH CHECK` corretos) e `role_table_grants` + `pg_roles` (`app_estoque` sem bypass, `auth_service` mantém bypass escopado). Os 3 confirmados.

**Verificação:**
- Suíte completa: 249/249 + `test_auth.py` isolado (6/6) = **255/255 passando**, incluindo depois de aplicar o RLS novo (cadastro/login continuam funcionando via bypass do `auth_service`, como esperado)
- Testes novos: `test_tenant.py` (9 testes — permissão por perfil, validação de CNPJ com DV, isolamento entre tenants) e `test_usuarios_me.py` (8 testes — edição de nome, validações de upload de foto, 503 quando Storage não configurado)
- Bandit: 0 issues (rodado antes e depois da correção de RLS)

**Entregável:** arquivos novos (`app/modules/tenant/*`, `tests/test_tenant.py`, `tests/test_usuarios_me.py`, `migrations/014_configuracoes.sql`, `migrations/015_rls_tenants.sql`) + arquivos alterados (`app/db/models.py`, `app/core/storage.py`, `app/core/config.py`, `app/main.py`, `app/modules/usuarios/*`, `.env.example`) + zip completo do projeto. Frontend desta etapa (as 4 abas da tela de Configurações) fica pra próxima entrega.


## Etapa 37 (continuação) — Tela de Configurações (frontend)

**Estrutura final:** 2 abas visíveis a todos os perfis (Minha Conta, Empresa) e 2 restritas a admin (Usuários, Estações de Impressão) — mesma visibilidade que os itens de Usuários/Estações já tinham como itens separados de menu antes desta etapa; a mudança foi só de *onde* moram, não de *quem* acessa. "Configurações" entra no menu principal pra todo mundo, já que Minha Conta é universal. Deep-link por query string (`/configuracoes?aba=usuarios`) — as antigas rotas `/usuarios` e `/estacoes` viraram redirects client-side pra não quebrar favoritos.

**Componentização:** conteúdo de `app/(dashboard)/usuarios/page.tsx` e `app/(dashboard)/estacoes/page.tsx` extraído sem alteração de lógica pra `components/configuracoes/AbaUsuarios.tsx` e `AbaEstacoes.tsx` (só removido cabeçalho de página, redundante dentro de uma aba). `AbaMinhaConta.tsx` e `AbaEmpresa.tsx` são novos, ligados aos 3 endpoints da parte de backend desta etapa.

- **Minha Conta:** nome editável (`PATCH /usuarios/me`) e foto de perfil (`POST /usuarios/me/foto`, mesmo padrão multipart já usado em imagem de produto). Card de troca de senha reaproveita a mesma chamada (`POST /auth/trocar-senha`) que já existia isolada em `/trocar-senha`, agora também disponível aqui pra troca voluntária (não só forçada por senha provisória).
- **Empresa:** leitura por qualquer perfil, edição (nome + CNPJ) só admin — reflete exatamente a permissão do backend. `segmento_slug` sempre somente-leitura na tela (decisão de produto, não limitação técnica — documentado no backend).
- `auth-context.tsx` ganhou `atualizarNome()`: sem isso, trocar o nome só refletiria na saudação da Home no próximo login (o JWT da sessão atual não é reemitido por uma troca de nome, mesma limitação já documentada pra `deve_trocar_senha`).

**Bug real encontrado e corrigido durante a integração:** `lib/api.ts` assumia que `detail` de erro do backend era sempre string. Os novos validators desta etapa (CNPJ com dígito verificador, nome mínimo) retornam 422 no formato padrão do Pydantic — uma lista de objetos `{loc, msg, type}`, não uma string. Sem correção, `ApiError.message` viraria esse array, e qualquer componente renderizando `{erro}` direto quebraria em runtime (React não aceita array de objetos como filho). Esse bug já existia antes desta etapa (outros `field_validator` no projeto, como força de senha no registro, tinham o mesmo problema) mas nunca tinha sido acionado organicamente pela UI. Corrigido no cliente HTTP central — beneficia toda a aplicação, não só as telas novas. Bônus: Pydantic v2 prefixa `ValueError` de field_validator com `"Value error, "`; removido na mesma correção pra mensagem ficar limpa pro usuário (`"CNPJ inválido."` em vez de `"Value error, CNPJ inválido."`).

**Verificação:**
- `npx tsc --noEmit`: 0 erros
- `npm run build`: sucesso, lint interno do Next sem warnings, `/configuracoes` gerado (12.9kB), `/usuarios` e `/estacoes` viraram redirects leves (~365B)
- Teste end-to-end manual dos 3 endpoints novos contra o backend local rodando de verdade (registro → login → `GET/PATCH /usuarios/me` → `GET/PATCH /tenant`, incluindo o caso de CNPJ inválido retornando 422 com a mensagem limpa) — contratos batendo exatamente com os tipos TypeScript
